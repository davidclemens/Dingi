classdef (SharedTestFixtures = { ...
        ...
        }) rdivide_test < matlab.unittest.TestCase
    % rdivide_test  Unittests for DataKit.Units.dimension.rdivide
    % This class holds the unittests for the DataKit.Units.dimension.rdivide
    % method.
    %
    % It can be run with runtests('Tests.DataKit.Units.dimension.rdivide_test').
    %
    %
    ...
    methods (Test)
        ...
        function testValidDimensions(testCase,In1,In2)
            ...
            elseif isDimension(1) && ~isDimension(2)
                expected = inputs{1};
            elseif ~isDimension(1) && isDimension(2)
                dims        = inputs{2}.Dimensions;
                exponents  	= inputs{2}.Degrees;
                value       = ['1/(',strjoin(strcat(dims,{'^'},arrayfun(@num2str,exponents,'un',0)),'*'),')'];
