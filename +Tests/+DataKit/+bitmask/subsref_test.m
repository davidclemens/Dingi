classdef (SharedTestFixtures = { ...
    ...
    properties (TestParameter)
        ...
        % {IndexType}_{SubscriptType}_{Index1Shape}_{Index2Shape}
        % Where:
        %   IndexType: prop (property), meth (method)
        %   SubscriptType: par ('()'), dot ('.'), brace ('{}') 
        %   Index1Shape: E (empty), S (scalar), V (vector)
        subDot = struct(...
            'prop_dot_E', 	struct(...
                                'type',     {'.'},...
                                'subs',     {'Bits'},...
                                'exp',      uint8([17,24,5;4,8,20])),...
            'prop_dot_S', 	struct(...
                                'type',     {'.','()'},...
                                'subs',     {'Bits',{4}},...
                                'exp',      uint8(8)),...
    ...
    methods (Test)
        ...
        function testDotSubsrefProperty(testCase,subDot)
            
            subs    = {subDot(:).type;subDot(:).subs};
            act     = subsref(testCase.obj,substruct(subs{:}));
            exp     = subDot.exp;
