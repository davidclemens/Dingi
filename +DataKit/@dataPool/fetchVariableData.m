...
%
    ...
    if any(poolIdx > obj.PoolCount)
        error('Dingi:DataKit:dataPool:fetchVariableData:invalidPoolIndex',...
            'Data pool index exceeds number of pools.')
    end
    if any(variableIdx > cat(1,obj.Info(poolIdx).VariableCount))
        error('Dingi:DataKit:dataPool:fetchVariableData:invalidVariableIndex',...
    ...
    for ii = 1:numel(poolIdx)
        switch obj.Info(poolIdx(ii)).VariableReturnDataType(variableIdx(ii))
            ...
            otherwise
                error('Dingi:DataKit:dataPool:fetchVariableData:unknownVariableReturnType',...
